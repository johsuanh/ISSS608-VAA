...
pacman::p_load(readxl, gifski, gapminder,
               plotly, gganimate, tidyverse, ggthemes)
```
...
We first use `read_xls` to import the document. However, the imported data have few issues to transform:
...
```{r}
globalpop_original <- read_xls("data/GlobalPopulation.xls",sheet="Data") 
head(globalpop_original)
```

Hence, we use `mutate_each_()` of `dplyr` package to convert all character data type into factor, and use `mutate` of `dplyr` package to convert data values of Year field into integer.
...
ggplot(globalPop, aes(x = Old, y = Young,
  ...
  labs(title = 'Global Population Change from 1997 to 2050',
...
bp <- globalPop %>%
  ...
  layout(showlegend = FALSE,
         title = list (text = "Global Population Change from 1997 to 2050",
