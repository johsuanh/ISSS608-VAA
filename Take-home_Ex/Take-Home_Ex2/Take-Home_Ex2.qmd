...
pacman::p_load(tidyverse, lubridate, gridExtra,
               ...
               ggiraph, plotly,knitr,ggiraph,ggstatsplot,readxl,
...
-   **Make-Over Suggestion**:
    ...
    1.  To improve the graph, we'll avoid direct category comparisons and instead use **`newggslopegraph`** from **CGPfunctions** to illustrate trends over multiple years. Labels will display both amounts and category proportions for clarity.
    2.  Additionally, highlighting data points for 2020 and 2022 could reveal variations during the COVID period (2020–2022).
...
sum_export_by_country$Country <- factor(sum_export_by_country$Country, 
                                   levels = c("UK","Australia","Japan",
                                               "China","USA"),
                                   ordered = TRUE)
avg_export_per_year$Country <- factor(avg_export_per_year$Country, 
                                   levels = c("UK","Australia","Japan",
                                               "China","USA"),
                                   ordered = TRUE)
# main line chart
...
# barchart
p2 <-ggplot(avg_export_per_year, 
            aes(x = Country, y= avg_annual_export,fill = Country)) +
  ...
  labs(y = "Average Yearly Export Value (S$ Bil)",
       caption="Source: singstat.gov.sg") +
  coord_flip() +
  theme_minimal() +
  theme(
...
Since there are 12 service types which make all graphs appear tiny and difficult to read, we can use Shiny and dropdown lists to resolve this issue. In the Shiny App version, the side graphs of p2 and p3 are replaced with a bar chart to show the distribution of sectors more intuitively.

To minimize data processing steps when loading the Shiny app, we first need to export the processed data to our folder and then import it to app.R (the Shiny file).
...
[Click here to view the Shiny app version](https://johsuanh.shinyapps.io/Take-Home_Ex2/)
...
-   **Transport**:
    ...
    As the line graph shows, transport service fees (which include carriage for both goods and travelers) began increasing in 2020 and peaked in 2022. Since many countries had travel restrictions during 2020–2022, the high fees were likely due to surging goods carriage costs. Hence, after 2022, the transport export gradually decreased.

-   **Financial & Manufacturing Services on physical inputs owned by others**:

    We observed a sharp decline in financial service exports in 2020, coinciding with the official UK Brexit (JAN 2020), which led to financial institutions moving away from the UK. However, there was also notable growth in UK's Manufacturing Services on physical inputs owned by others from 2021. After Brexit, the UK-Singapore Free Trade Agreement (Dec 2020) may have contributed to increased re-export activity and related services.
:::
...
-   **Remainder and Value**: After January 2020, both imports and exports to the US showed noticeably more volatile patterns in the panels compared to the pre-2020 period.
...
The cycle plot reveals trends, seasonality, and cycles. It shows a seasonal pattern, with trade amounts dipping in February and peaking in March. The trend also indicates a gradual increase in trade over the years.

However, the cycle component is irregular and highly volatile, with repeating variations that lack a clear, predictable pattern.
...
m_usa_long %>%
  ACF(value) %>% 
  autoplot()+
  labs(title = "Autocorrelation Function of US-Singapore Trade Amounts",
       caption = "Source: singstat.gov.sg")+
  theme(plot.title = element_text(size = 11, hjust = 0.5, face = 'bold'),
        plot.subtitle = element_text(size = 8, hjust = 0.5),
        axis.title = element_text(size = 8),
...
m_usa_long %>%
  PACF(value) %>% 
  autoplot()+
  labs(title = "Partial Autocorrelation Function of US-Singapore Trade Amounts",
       caption = "Source: singstat.gov.sg")+
