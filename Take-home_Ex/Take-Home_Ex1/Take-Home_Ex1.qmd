...
-   **Distance travel & Cost per Nautical Mile** :According to Kaggle, "**Distance_Traveled_nm**" represent total engine power and distance traveled by the ship. These values will be divided by weekly_voyage_count. In addition, cost per Nautical Mile will be derived for performance evaluation.
    ...
    # create log1p(cost per nm)
    ship_cleaned <- ship_cleaned %>%
      mutate(log1p_Cost_per_nm = log1p(Cost_per_Voyage_k / distance_travel_per_V))
...
## 5.1 **Financial Performance:** Exploring Average Revenue and Net Margin Across Ship Types
...
-   **Container Ship**: Experiences peaks in November and January, coinciding with high-demand periods like Black Friday, Christmas, and Chinese New Year. Profit margin is 21.27% with revenue of 524.55K.
...
p2 <- ggplot() + 
  geom_line_interactive(data = ship_avg, 
                        aes(x = New_Date, y = avg_margin,
                            color = Ship_Type,tooltip = round(avg_margin_yearly,2),
                            data_id = Ship_Type), size = 1) +
...
## 5.2 **Financial Performance:** Seasonal Profit Trends by Ship Type: A Closer Look

Based on the previous plot, we analyze operational profit by ship type. The "Average Load Percentage" tab suggests that lower profits may result from reduced load percentages, leading to decreased revenue and higher operational costs.

-   **Tanker**: Revenue dips in September, January, and May are linked to large tankers operating below the quarterly average load percentage of 75%–76%, reducing overall profit.

-   **Bulk Carrier**: In January (Q1) and November (Q4), both medium and large bulk carriers show lower loading rates, correlating with decreased profitability.
...
## 5.3 **Financial Perfomance:** Sailing Towards Profit: Which Ship Types Lead the Way?

Revenue, profit margin, and cost per nautical mile are key indicators for assessing a ship's financial performance. Higher revenue and profit margin reflect the ship’s ability to generate income, while cost per nautical mile evaluates operational efficiency, considering expenses like fuel, crew wages, and maintenance. A lower cost per nautical mile signifies more efficient operations, directly enhancing profitability and competitiveness.

The plot below shows bar charts for these indicators, with average monthly growth rates indicating potential growth in different ship types. The chart is sorted by revenue, with values ranging from **500K to 595K**. Although revenue is similar across ship types, profit margins show significant variation, ranging from **7.4% to 36.6%**. Additionally, **large fish carriers** and **large fish tankers** show a high monthly growth rate, which is primarily driven by negative operations in the prior month, rather than positive growth, highlighting the potential for recovery.
...
ship_analysis <- ship_cleaned %>%
  group_by(type_size, New_Date) %>%
  summarise(
    mean_revenue = mean(Revenue_per_Voyage_k, na.rm = TRUE),
    mean_log1p_Cost_nm = mean(log1p_Cost_per_nm, na.rm = TRUE),
    mean_profit = mean(Profit_per_Voyage, na.rm = TRUE),
    mean_margin = mean(Margin_per_Voyage, na.rm = TRUE),
    .groups = "drop"  # Ensures output is ungrouped
  ) %>%
  arrange(type_size, New_Date) %>%  # Ensure sorted by type_size & month
  group_by(type_size) %>%  # Group again for growth rate calculation
  mutate(
    revenue_growth = (mean_revenue - lag(mean_revenue)) / lag(mean_revenue),
    profit_growth = (mean_profit - lag(mean_profit)) / lag(mean_profit)
  ) %>%
...
ship_summary <- ship_analysis %>%
  group_by(type_size) %>%
  summarise(mean_revenue_all = mean(mean_revenue, na.rm = TRUE)) %>%
  arrange(mean_revenue_all)

# Create the long format dataframe for rev & p >> column 1
ship_PnL <- ship_analysis %>%
    group_by(type_size) %>%
    summarise(
        avg_revenue = mean(mean_revenue, na.rm = TRUE),
        avg_profit = mean(mean_profit, na.rm = TRUE)
    ) %>%
...
ship_margin_growth <- ship_analysis %>%
    group_by(type_size) %>%
    summarise(
        avg_p_growth = mean(profit_growth, na.rm = TRUE)*100,
        avg_margin =mean(mean_margin, na.rm = TRUE)

    ) %>%
    pivot_longer(cols = c(avg_margin, avg_p_growth), 
                names_to = "metric", 
                values_to = "value") 

# Create the long format dataframe for growth rate and margin >> column 3
ship_op_efficiency <- ship_analysis %>%
    group_by(type_size) %>%
    summarise(
        avg_cost_nm = mean(mean_log1p_Cost_nm, na.rm = TRUE)
    ) %>%
    pivot_longer(cols = c(avg_cost_nm), 
                names_to = "metric", 
                values_to = "value") 

ship_metrics_summary <- ship_analysis %>%
  group_by(type_size) %>%
  summarise(
    avg_revenue = mean(mean_revenue, na.rm = TRUE),
    avg_profit = mean(mean_profit, na.rm = TRUE),
    avg_margin = mean(mean_profit, na.rm = TRUE)/mean(mean_revenue, na.rm = TRUE),
    avg_p_growth = mean(profit_growth, na.rm = TRUE),
    avg_cost_nm = mean(mean_log1p_Cost_nm, na.rm = TRUE)
  )
...
ship_combined <- bind_rows(
    ...
    ship_margin_growth %>% mutate(category = "Margin & Growth% "),
  ...
  mutate(
    tooltip = case_when(
      ...
      metric == "avg_profit" ~ sprintf("Ship Type: %s\nProfit: %.2f K", type_size, value),
      metric == "avg_margin" ~ sprintf("Ship Type: %s\nProfit Margin: %.2f%%", type_size, value),
      metric == "avg_p_growth" ~ sprintf("Ship Type: %s\nProfit Growth: %.2f%%", type_size, value),
...
p <- ggplot(ship_combined, 
  ...
  geom_bar_interactive(stat = "identity",
                       ...
                       width = 0.6) +
  coord_flip() +
  facet_wrap(~ factor(category, levels = c("Revenue & Profit", "Margin & Growth% ","Cost Efficiency")),
             scales = "free_x") +  # Separate the two sections
  labs(
    title = "Sailing Towards Profit: Which Ship Types Lead the Way?",
    subtitle = "The Medium Fish Carrier reigns as the top performer, with a revenue of 595K and a profit margin of 57%. In contrast, the Medium Bulk Carrier underperforms, generating 509K in revenue and the lowest margin at 8.38%",
    x = "Ship Type",
    y = "",
    fill = "Metric"
  ) +
...
# Create the long format dataframe for rev & p >> column 1
ship_PnL <- ship_cleaned %>%
    group_by(type_size) %>%
    summarise(
        avg_revenue = mean(Revenue_per_Voyage_k, na.rm = TRUE),
        avg_profit = mean(Profit_per_Voyage, na.rm = TRUE)
    ) %>%
    pivot_longer(cols = c(avg_revenue, avg_profit), 
                names_to = "metric", 
                values_to = "value") 
...
p <- ggplot(ship_combined, 
  ...
  labs(
    title = "Sailing Towards Profit: Which Ship Types Lead the Way?",
    subtitle = "The Medium Fish Carrier reigns as the top performer, with a revenue of 595K and a profit margin of 57%.\n In contrast, the Medium Bulk Carrier underperforms, generating 509K in revenue and the lowest margin at 8.38%",
    x = "Ship Type",
    y = "",
    fill = "Metric"
  ) +
...
## 5.5 **Operational Performance**: Which Ship & Route Types Have the Most Turnaround Time?
...
Our analysis reveals that long-haul and transoceanic routes have the longest median turnaround times, with an average of 43 hours. The Kruskal-Wallis test (p-value = 0.076) suggests a marginally significant difference in turnaround times across ship types, while Dunn's Test identifies significant differences between bulk carriers and fish carriers, as well as tankers. This indicates that ship type are key factors in turnaround efficiency. Reducing turnaround time is essential for optimizing port operations and minimizing associated costs, ultimately improving overall performance.
...
print(mean_tat)
dunn.test(ship_cleaned$Turnaround_Time_hours, ship_cleaned$Ship_Type, kw = TRUE)
```
:::

# 6 Executive Summary

This analysis of ship performance in the Gulf of Guinea identifies key factors impacting operational cost and financial outcomes, with a focus on **load percentage**, **engine type**, **maintenance status**, **turnaround time** and **profitability**. The key findings are:
...
-   As stricter marine regulations impose higher maintenance demands, ships with **Diesel** and **Heavy Fuel Oil engines** in **critical maintenance status** incur significantly higher operational costs. A **Kruskal-Wallis test** further confirmed that while neither engine type nor maintenance status alone had a significant effect, there was a **strong interaction effect** (p-value = 0.001), emphasizing the need to consider both factors together when managing operational expenses.

-   In terms of **turnaround time (TAT)** , long-haul and transoceanic routes showed the longest median TAT at **43 hours**. Statistical testing indicated a marginal difference in TAT across ship types, with significant differences between **bulk carriers**, **fish carriers**, and **tankers**.

-   Finally, key financial indicators—**revenue**, **profit margin**, and **cost per nautical mile**—reveal variations in performance. Despite similar revenue levels, profit margins vary significantly, suggesting opportunities for cost optimization and efficiency improvements, particularly in ships with **high monthly growth rates**, driven by prior negative performance.
