...
```{r}
pacman::p_load(tidyverse,DT,tseries,lubridate,knitr,tsibble,fable,feasts,ggthemes, forecast,urca,kableExtra)
...
time_resolution_table %>%
  ...
  theme(
    ...
    strip.text = element_text(color = "#1A1A1A",size =6),
    legend.position = 'top',
...
ts_monthly_tsibble %>%
  gg_subseries(value)+
  labs(title="Cycle Plot", subtitle="",y = paste0(variable),x="")+
  theme_classic()+
  scale_color_manual(values = c("#4D4D4D"))+
  theme(
    plot.title = element_text(face = "bold", hjust = 0.5,color="#4D4D4D"),
...
model_matrix <- matrix %>%
  ...
  rename(RMSE_ts = RMSE, AIC_tr = AIC, AICc_tr = AICc, BIC_tr = BIC, Model = .model)

kable(model_matrix) %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
  row_spec(0, background = "#C4C8FF", color = "#4D4D4D") 
...
comparison_table_test <- 
  ...
  select(c(Year, Month, Model, `Forecasted Value`, value)) %>%
  rename(`Actual Value` = value) %>%
  mutate(`Forecasted Value` = round(`Forecasted Value`,2))%>%
  select(-c(year_month))

datatable(comparison_table_test, 
          options = list(
            scrollY = "250px",       # Set the scroll height
            scrollCollapse = TRUE,   # Collapse empty space when scrolling
            paging = FALSE,          # Disable paging for continuous scrolling
            searching = TRUE,        
            headerCallback = JS(
...
forecast_final_results <- fit_chosen_model %>%
...
```{r}
# Plot the forecast
autoplot(forecast_final_results, level = c(95)) +
  autolayer(forecasting, series = "Actual", color = "#4D4D4D") + 
...
autoplot(residuals, .vars = .resid) + 
  labs(title = "Residuals of Forecasting Model",
       x = "", 
       y = "Residuals") +
  theme_classic()+
    theme(
...
```{r}
comparison_table <- forecast_final_results %>%
  select(year_month, .model, value, .mean) %>%
...
comparison_table <- comparison_table %>%
  select(Year, Month, .model,`Forecasted Value`) %>%
  rename(Model = .model)
...
![](prototype/Protoype-2.png){fig-align="center"}

## Correlograms

![](prototype/Protoype-3.png){fig-align="center"}

## Forecasting - Model Training

![](prototype/Protoype-4.png){fig-align="center"}

## Forecasting - Model Selection

![](prototype/Protoype-5.png){fig-align="center"}
:::
