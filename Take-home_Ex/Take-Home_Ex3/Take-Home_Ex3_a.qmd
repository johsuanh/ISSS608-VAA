...
#### 2.2.4.3 **Remove Marina Barrage Station Data**
...
Since the missing values are spread throughout the dataset, and because this is time-series weather data, we cannot simply remove entire rows without disrupting further visualization. To avoid analysis errors, interpolation is a more suitable approach for handling these missing values.

Below are the methods we used to handle missing values for different attributes:
...
-   **No temperature data:** interpolate mean temperature using the `zoo` package by calculating moving averages across 7 days \>\> 15 days \>\> 30 days
...
-   **No wind speed data:** interpolate wind speed using the `zoo` package by calculating moving averages across 7 days \>\> 15 days \>\> 30 days \>\> 45 days \>\> 60 days

    -   The stations at Jurong (West), Tai Seng, and Clementi have continuous missing values extending beyond 30 days, so we expanded the moving average window to 60 days.
...
In Shiny APP, we will offer three time resolutions: daily, weekly and monthly data.
